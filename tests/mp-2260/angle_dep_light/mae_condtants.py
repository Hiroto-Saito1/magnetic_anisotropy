...
class Mae_constants:
    ...
    def plot_phi_dep(
        ...
        plt.plot(np.linspace(0, 180, phi_fitted_mesh), phi_dep_fitted, color="black")
        plt.grid()
        plt.xticks([0, 45, 90, 135, 180], ["0", "45", "90", "135", "180"])
        plt.xlabel("$\phi$ [deg]")
        plt.ylabel("$dE$ [$\mathrm{meV}/f.u.$]")
    ...
    def plot_theta_dep(
        ...
        plt.plot(
            np.linspace(0, 180, theta_fitted_mesh), theta_dep_fitted, color="black"
        )
        plt.grid()
        plt.xticks([0, 45, 90, 135, 180], ["0", "45", "90", "135", "180"])
        plt.xlabel("$\\theta$ [deg]")
    ...
    def calc_mae_constants(self, theta_fit, phi_fit):
        ...
        K12 = phi_fit[0][0]
        K3 = phi_fit[0][1]
        print("MAE constants [meV]")
        print("K1: {}, K2: {}, K2: {}, K3: {}".format(K1, K23 - K3, K12 - K1, K3))
        K1_err = theta_fit[1][0]
        K23_err = theta_fit[1][1]
        K12_err = phi_fit[1][0]
        K3_err = phi_fit[1][1]
        print(
